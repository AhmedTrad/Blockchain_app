...
def obten_posts():
    """
    Funcion para recuperar la cadena de un nodo de cadena de bloques, 
    analizar los datos y almacenarlos localmente.
    """
    ...
    if respuesta.status_code == 200:
        ...
        for bloque in cadena["cadena"]:
            for tx in bloque["transacciones"]:
                ...
                contenido.append(tx)
        global posts
        posts = sorted(contenido, key=lambda k: k['timestamp'],
                       reverse=True)
...
def index():
    ...
    return render_template('index.html',
                           title='SCPN: Compartiendo contendio de forma descentralizada',
                           posts=posts,
                           node_address=DIRECCION_NODO_CONECTADO,
                           readable_time=timestamp_a_cadena)


@app.route('/enviar', methods=['POST'])
def enviar_textarea():
    """
    Endpoint para crear una nueva transaccion a traves de nuestra aplicacion.
    """
    ...
    requests.post(direccion_nueva_tx,
                  json=post_object,
